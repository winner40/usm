...
{	
	...
	case 4: {
			int ret = uffd_eMod(ctx->mm, uffdio_palloc.addr, uffdio_palloc.arg.uaddr, uffdio_palloc.write);
			if (likely(ret == 1)) {
...
SYSCALL_DEFINE1(userfaultfd, int, flags)
{
	...
	if ((flags & UFFD_USM) !=0) {
		...
		current->mm->def_flags |= VM_UFFD_MISSING | VM_UFFD_SWAPPING |VM_UFFD_WP /* | VM_PFNMAP*/; /* | VM_IO;  | VM_LOCKED | VM_IO;		 UFFD_USM !  | No MINOR man meh*/
